...
#字典：六亲定义
liuqin_Dict = {0:"兄弟", 1:"父母", 2:"官鬼", 3:"子孙", 4:"妻财"}
'''
#第一个属性为“我”
#同我者兄弟
...
#获得之卦的结果，只选取动爻部分
def biangua(waidongyao_1, neidongyao_2,waigua,neigua):
    waidongyao = (waidongyao_1[0], waidongyao_1[1], waidongyao_1[2])
    neidongyao = (neidongyao_2[0], neidongyao_2[1], neidongyao_2[2])
...
#本卦宫五行干支
def guaGong(wuxing,BenGuaGong):
    dizhiup = SJG.BaGuaUp(BenGuaGong.number)
    dizhidown = SJG.BaGuaDown(BenGuaGong.number)
    activeGanZhi = [0, 0, 0, 0, 0, 0]
    for counters in range(3):
        activeGanZhi[counters] = dizhiup[counters]
    for counters in range(3):
        activeGanZhi[counters + 3] = dizhidown[counters]
    liuqin = AnLiuQin(wuxing, dizhiup, dizhidown)
    return liuqin,activeGanZhi

#本卦之卦卦名
def find_names(numup_ben,numdown_ben,numup_zhi,numdown_zhi):
    benguaName = liushisi_Gua.get((numup_ben, numdown_ben))
    zhiguaName = liushisi_Gua.get((numup_zhi, numdown_zhi))
    return benguaName,zhiguaName